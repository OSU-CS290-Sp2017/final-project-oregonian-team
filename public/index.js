...
function insertNewPhoto() {
  ...
  if (itemName.trim() && itemDescription.trim() && itemLocation.trim() && itemImage.trim()) {
    ...
    if (category) {
      ...
      storeCategoryItem(category, itemName, itemDescription, itemLocation, itemImage, function (err) {
        if (err) {
 ...
 */
function storeCategoryItem(category, name, description, location, image, callback) {
